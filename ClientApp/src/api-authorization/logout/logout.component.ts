    ...
    switch (action.path) {
      case LogoutActions.Logout:
        if (!!window.history.state.local) {
          ...
          this.message.next('A Saída não foi feita de dentro da página.');
        }
      ...
      case LogoutActions.LoggedOut:
        this.message.next('Você efetuou Saída com sucesso!');
        break;
      default:
    ...
    if (isauthenticated) {
      ...
      switch (result.status) {
        ...
        case AuthenticationResultStatus.Fail:
          ...
          break;
        default:
          throw new Error('Status de resultado de autenticação inválida.');
      }
    } else {
      this.message.next('Você efetuou Saída com sucesso!');
    }
  ...
  private async processLogoutCallback(): Promise<void> {
    ...
    switch (result.status) {
      case AuthenticationResultStatus.Redirect:
        ...
        throw new Error('Não deve redirecionar.');
      case AuthenticationResultStatus.Success:
